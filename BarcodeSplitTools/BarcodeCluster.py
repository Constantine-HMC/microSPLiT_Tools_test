...
def CD_HIT(barcode_fasta, cdhit, RAM, threads, outdir, name, threshold):
    ...
    if not return_number == 256:
        ...
        with open(out_fasta, 'r') as cluster_fasta:
            with open(os.path.join(outdir, name + '.cell_barcode.txt'), 'w') as cell_barcode_txt:
                n = 0
                for line in cluster_fasta:
                    if line.strip().startswith('>'):
                        n += 1
                        cell_barcode_txt.write('>cell_'+str(n)+'\n')
                    else:
                        cell_barcode_txt.write(line)
        os.system('rm -rf %s' % (out_fasta))
